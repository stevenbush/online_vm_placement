#!/usr/bin/python
# Filename: submit_generate_jobs.py

import glob, string, os, sys, traceback

if len(sys.argv) < 4:
    print 'No input path, resultpath and log path specified.'
    print 'please enter \"submit_generate_jobs.py inputpath resultpath logpath\"'
    sys.exit()
...
try:
    ...
    for pathvalue in filepath:
        ...
        cmd = "bsub -q 8nh -e %s -o %s \'cd /afs/cern.ch/work/j/jishi/private/experiment/googleclusterdata/ && python /afs/cern.ch/work/j/jishi/private/experiment/googleclusterdata/script/generate_jobs.py %s %s > %s 2>&1\'" % (logpath + '/' +jobname + '.elog', logpath + '/' +jobname + '.olog', pathvalue, resultpath, logpath + '/' +jobname + '.log')
        print cmd
        #os.system(cmd)
