...
if len(sys.argv) < 4:
    ...
    print 'please enter \"submit_generate_workload_job.py inputpath resultpath logpath\"'
    sys.exit()
...
try:
    ...
    for pathvalue in filepath:
        ...
        cmd = "bsub -q 8nh -e %s -o %s \'cd /afs/cern.ch/work/j/jishi/private/experiment/googleclusterdata/ && python /afs/cern.ch/work/j/jishi/private/experiment/googleclusterdata/script/plot/generate_raw_workload.py %s %s > %s 2>&1\'" % (logpath + '/' + jobname + '.elog', logpath + '/' + jobname + '.olog', pathvalue, resultpath, logpath + '/' + jobname + '.log')
        # print cmd
        os.system(cmd)
