...
if len(sys.argv) < 5:
    print 'No input file, plot name, out path and time period.'
    print 'python plot_proportion.py input_file plot_name output_path time_period'
    sys.exit()
...
try:  
    ...
    # print result_host_list
    
    fig, ax = plt.subplots()

    plt.plot(result_host_list, label='BestFit Host Demand')
    ...
    plt.grid(True)    
    ax.set_xticks(range(0, len(result_host_list), 2 * (24 * 3600) / period_seconds))
    ax.set_xticklabels(range(0, 1 + len(result_host_list) / ((24 * 3600) / period_seconds), 2))
