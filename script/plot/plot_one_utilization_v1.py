...
for inputfile in host_path:
    ...
    for i in range(len(ORA_result_host_list)):
        ORA_result_host_list[i] = result_cpuload_list[i] / ORA_result_host_list[i]
...
for inputfile in host_path:
    ...
    for i in range(len(ARP_result_host_list)):
        ARP_result_host_list[i] = result_cpuload_list[i] / ARP_result_host_list[i]
...
for inputfile in host_path:
    ...
    for i in range(len(ARP_result_host_list)):
        ROBP_result_host_list[i] = result_cpuload_list[i] / ROBP_result_host_list[i]
     
fig, ax = plt.subplots()
     
# plt.plot(result_cpuload_list, 'k', label='Actual Workload Demand')

# plt.plot(bestfit_result_host_list, label='BestFit Host Demand')

plt.plot(ORA_result_host_list, 'b', label='ORA Host Demand')

plt.plot(ARP_result_host_list, 'g', label='ARP Host Demand')

plt.plot(ROBP_result_host_list, 'r', label='ROBP Host Demand')
     
# plt.tight_layout()
     
plt.xlabel('Time(day)')
plt.ylabel('Average Resource Utilization (every ' + str(string.atoi(time_period) // 3600) + ' hours)')
