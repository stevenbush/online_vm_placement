...
public class OneDimensional_ROBP_Event_Processor extends
	...
	private void fill(ROBP_Host host) throws Exception {
		while ((host.getCpu_utilization() <= 3.0 / 4.0)
			...
			for (ROBP_Host T_host : TO_Bin_list) {
				for (String vm_id : T_host.getVm_list()) {
					VM current_vm = vm_table.get(vm_id);
					if (current_vm.getCpu_demand() <= (1 - host
							.getCpu_utilization())) {
						find_T_item = true;
						T_item = current_vm;
						src_host = T_host;
	...
	private void merge(ROBP_Host host1, ROBP_Host host2) throws Exception {
		...
		if (host1.getM_num() > 0 && host1.getH_num() == 0
				&& host1.getB_num() == 0 && host1.getL_num() == 0
				&& host1.getS_num() == 0 && host1.getT_num() == 0
				&& host1.getCpu_utilization() <= 3.0 / 4.0
				&& host2.getM_num() > 0 && host2.getH_num() == 0
				&& host2.getB_num() == 0 && host2.getL_num() == 0
				&& host2.getS_num() == 0 && host2.getT_num() == 0
		...
		if (host1.getS_num() > 0 && host1.getCpu_utilization() <= 3.0 / 4.0
				&& host2.getS_num() > 0
				&& host2.getCpu_utilization() <= 3.0 / 4.0) {
			ROBP_Host SO_Bin = null;
			ROBP_Host S_Bin = null;
			if (host1.getS_num() > 0 && host1.getH_num() == 0
					&& host1.getB_num() == 0 && host1.getL_num() == 0
					&& host1.getT_num() == 0 && host1.getM_num() == 0
					&& host2.getS_num() > 0 && host2.getH_num() == 0
					&& host2.getB_num() == 0 && host2.getL_num() == 0
					&& host2.getT_num() == 0 && host2.getM_num() == 0) {
				...
				if (S_Bin.getT_num() > 0) { // if the host containing T-item we
					...
					for (String iterable_vm_id : S_Bin.getVm_list()) {
						...
						if (current_item.getCpu_demand() <= 1.0 / 3.0
								&& current_item.getCpu_demand() > 1.0 / 4.0) {
							T_item = current_item;
							break;
						}
					}
					...
					for (ROBP_Host iterable_host : this.host_list) {
						if ((T_item.getCpu_demand() <= 1 - iterable_host
								.getCpu_utilization())
					...
					while (S_Bin.getCpu_utilization()
							+ SO_Bin.getCpu_utilization() > 1.0) {
	...
	private void movethegap(ROBP_Host host) throws Exception {
			...
			for (ArrayList<String> group : groups) {
				...
				for (String vm_id : group) {
					group_size = group_size
							+ vm_table.get(vm_id).getCpu_demand();
				}
				Boolean find_host = false;
				for (ROBP_Host iterable_host : this.host_list) {
		...
		for (String vm_id : vm_id_list) {
			VM current_vm = vm_table.get(vm_id);
			if (current_vm.getCpu_demand() <= 1.0 / 3.0
					&& current_vm.getCpu_demand() > 1.0 / 4.0) { // T-item
				Boolean find_host = false;
				for (ROBP_Host iterable_host : this.host_list) {
					if (((1.0 - iterable_host.getCpu_utilization()) >= current_vm
							.getCpu_demand())
							&& !this.MO_Bin_list.contains(iterable_host)
							&& iterable_host != host) {
	...
	private void insert(VM vm) throws Exception {
			...
			if (!S_Bin_list.isEmpty()) {
				for (ROBP_Host iterable_host : S_Bin_list) {
					for (String vm_id : iterable_host.getVm_list()) {
						VM current_vm = vm_table.get(vm_id);
						if (current_vm.getCpu_demand() <= (1 - new_host
								.getCpu_utilization())
								&& current_vm.getCpu_demand() <= 1.0 / 2.0
								&& current_vm.getCpu_demand() > 1.0 / 3.0) {
			...
			if (!L_Bin_list.isEmpty()) {
				for (ROBP_Host iterable_host : L_Bin_list) {
					if (iterable_host.getS_num() == 0) {
						for (String vm_id : iterable_host.getVm_list()) {
							VM current_vm = vm_table.get(vm_id);
							if (current_vm.getCpu_demand() <= (1 - vm
									.getCpu_demand())
									&& current_vm.getCpu_demand() <= 2.0 / 3.0
									&& current_vm.getCpu_demand() > 1.0 / 2.0) {
								if (iterable_host.getCpu_utilization()
										+ vm.getCpu_demand() > 1) {
									this.movethegap(iterable_host);
								}
		...
		if (vm.getCpu_demand() <= 1.0 / 3.0 && vm.getCpu_demand() > 1.0 / 4.0) { // T-item
			Boolean find_host = false;
			for (ROBP_Host iterable_host : this.host_list) {
				if (((1.0 - iterable_host.getCpu_utilization()) >= vm
						.getCpu_demand())
						&& !this.MO_Bin_list.contains(iterable_host)) {
					iterable_host.add_vm(vm);
					this.update_bin_type(iterable_host);
	...
	private void underload_handler(ROBP_Host host) throws Exception {
		...
		for (String id : host.getVm_list()) {
			...
			if (current_vm.getCpu_demand() <= 3.0 / 4.0
					&& current_vm.getCpu_demand() > 2.0 / 3.0) { // B-item
				B_list.add(current_vm);
			}
			if (current_vm.getCpu_demand() <= 2.0 / 3.0
					&& current_vm.getCpu_demand() > 1.0 / 2.0) { // L-item
