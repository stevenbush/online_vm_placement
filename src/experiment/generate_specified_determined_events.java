...
public class generate_specified_determined_events {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		if (args.length < 3) {
			System.out.println("No input path, resultpath and ratio specified.");
			System.out.println("please enter \"generate_worst_events inputpath resultpath ratio\"");
			return;
		}
		...
		UniformIntegerDistribution determinedDistribution = new UniformIntegerDistribution(0, 1);
		...
		for (int i = 0; i < file_list.length; i++) {
			...
			try {
				...
				CSVWriter writer = new CSVWriter(new FileWriter(result_path + "/" + ratio + "-" + file_name),
				...
				for (String[] item_line : items_list) {
					if (!item_line[3].equals("4")) {
						...
						if (tiny_size <= 0.5) {
							...
							if (determinedDistribution.sample() == 0) {
								scalling_factor = upper_bound;
							} else {
								scalling_factor = lower_bound;
							}
							Double CPU = seed_value;
							Double MEM = seed_value * scalling_factor;
